...
def page_home():
    ...
    st.write("*Seuls les packages streamlit, numpy, matplotlib et pandas ont été utilisés")
...
def page_csv():
    ...
    def define_k():
        ...
        print(st.session_state['dataset_size_'])
    ...
    if st.session_state['choose_k_'] == True:
        ...
        if  k_choice_.button("Lancer Kmeans"):
            ...
            for i, (centroid, std_dev) in enumerate(zip(centroids, std_devs)):
                k_choice_.write(f"Cluster {i+1}: Centroïde = {centroid}, Écart-type = {std_dev}")
